...
module.exports = {
  getProfile: async (request, response) => {
    ...
    try {
      const checkEmail = await Profile.checkEmail(userId);
      if (checkEmail.length === 0) {
        throw new Error("User not found");
      }
      const profile =
        checkEmail[0].email === "gunturkurniawan238@gmail.com"
          ? await Profile.detailAdmin(userId)
          : await Profile.detailUsers(userId);
      misc.response(
        response,
        200,
        false,
  ...
  createProfile: async (request, response) => {
    try {
      ...
      let requireCheck = [];
      let data = {};
      if (checkEmail[0].email === "gunturkurniawan238@gmail.com") {
        const {
          user_id,
          province,
          province_name,
      ...
      const created = await Profile.storeProfile(checkEmail[0].email, data);
      const payload = {
        user: {
          id: created.insertId,
        },
  ...
  uploadAdmin: async (request, response, next) => {
    let error = false;
    if (request) {
      if (request.file) {
    ...
    try {
      if (error === false) {
        await Profile.uploadAdmin(photo, user_id);
        redisClient.flushdb();
        misc.response(response, 200, false, "Success upload profile Admin");
      }
    } catch (error) {
      console.error(error);
      misc.response(response, 500, true, "Server error");
  ...
  uploadUsers: async (request, response, next) => {
    let error = false;
    if (request) {
      if (request.file) {
        if (request.file.size >= 5242880) {
    ...
    try {
      if (error === false) {
        await Profile.uploadUsers(photo, user_id);
        redisClient.flushdb();
        misc.response(response, 200, false, "Success upload profile users");
      }
    } catch (error) {
      console.error(error);
      misc.response(response, 500, true, "Server error");
